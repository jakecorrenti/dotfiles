local hsluv = require("monokaipro.hsluv")
...
-- local ns = vim.api.nvim_create_namespace("monokaipro")
...
function util.debug(colors)
	colors = colors or require("monokaipro.colors")
	-- Dump unused colors
...
function util.onColorScheme()
	if vim.g.colors_name ~= "monokaipro" then
		vim.cmd([[autocmd! monokaipro]])
		vim.cmd([[augroup! monokaipro]])
	end
end
...
function util.autocmds(config)
	vim.cmd([[augroup monokaipro]])
	vim.cmd([[  autocmd!]])
	vim.cmd([[  autocmd ColorScheme * lua require("monokaipro.util").onColorScheme()]])

	if config.dev then
		vim.cmd([[  autocmd BufWritePost */lua/monokaipro/** nested colorscheme monokaipro]])
	end

	if config.flatTerm == true then
	else
		vim.cmd([[  autocmd TermOpen * setlocal winhighlight=Normal:NormalLB,SignColumn:SignColumnLB]])
	end

	if config.flatFloat == true then
	else
		vim.cmd(
			[[  autocmd FileType qf setlocal winhighlight=Normal:NormalLB,SignColumn:SignColumnLB,CursorLine:CursorLineLB]]
		)
	end

	for _, sidebar in ipairs(config.sidebars) do
		vim.cmd(
			[[  autocmd FileType ]] .. sidebar .. [[ setlocal winhighlight=Normal:NormalDB,SignColumn:SignColumnDB]]
		)
	end

	for _, float in ipairs(config.float) do
		vim.cmd([[  autocmd FileType ]] .. float .. [[ setlocal winhighlight=Normal:NormalLB,SignColumn:SignColumnLB]])
	end
...
function util.terminal(colors)
	-- dark
	vim.g.terminal_color_0 = colors.bg_light
